...
* FEATURE: [vmoperator](https://docs.victoriametrics.com/operator/): Add `k8s_version` label to `vm_app_version` metric. See [#2530](https://github.com/VictoriaMetrics/operator/pull/2530).
* FEATURE: [vmoperator](https://docs.victoriametrics.com/operator/): add a unified `operator_controller_errors_total{controller, namespaced_name, reason}` metric covering all reconcile failure categories (`get_object`, `parse_object`, `cancel_context`, `conflict`, `other`) across every controller, replacing several single-purpose counters that will be removed in a future release. See [#2536](https://github.com/VictoriaMetrics/operator/pull/2536).
...
* FEATURE: [vmoperator](https://docs.victoriametrics.com/operator/): propagate `volumeClaimTemplate.spec.volumeAttributesClassName` changes to existing `PersistentVolumeClaim`s. Removing `volumeAttributesClassName` once set is rejected, since it's immutable in Kubernetes. See [#2598](https://github.com/VictoriaMetrics/operator/issues/2598).
...
* BUGFIX: [vmoperator](https://docs.victoriametrics.com/operator/): fix reconciliation error on Kubernetes 1.29 caused by setting an empty `preStop` lifecycle handler. The native `Sleep` preStop action now requires Kubernetes >= 1.30, since the `PodLifecycleSleepAction` feature gate is not enabled by default on 1.29. See [#2522](https://github.com/VictoriaMetrics/operator/pull/2522).
...
* BUGFIX: [vmagent](https://docs.victoriametrics.com/operator/resources/vmagent/), [vmsingle](https://docs.victoriametrics.com/operator/resources/vmsingle/): create and remove RBAC resources in namespaces configured for each reconciler. See [#2540](https://github.com/VictoriaMetrics/operator/pull/2540).
...
* BUGFIX: [vmoperator](https://docs.victoriametrics.com/operator/): synchronize the operator's `ClusterRole` with kubebuilder markers. See [#2543](https://github.com/VictoriaMetrics/operator/pull/2543).
...
* BUGFIX: [vmoperator](https://docs.victoriametrics.com/operator/): remove the invalid `items` key under `dnsConfig` in the `VMAgent`, `VMAlertmanager`, `VMAnomaly`, `VMCluster`, `VLAgent`, `VLCluster`, `VTAgent` and `VTCluster` CRDs. Previously, while Kubernetes itself ignored this key, it could fail validation in strict JSON Schema consumers such as KubeVela. See [#2618](https://github.com/VictoriaMetrics/operator/issues/2618).
* BUGFIX: [VMRule CRD](https://docs.victoriametrics.com/operator/api/#v1beta1-vmrule): drop the leftover default value `""` from the `record` and `alert` fields. This default caused Kubernetes to add an empty `alert: ""` or `record: ""` to every rule, so GitOps tools such as ArgoCD always detected a diff and kept `VMRule` objects `OutOfSync`. This default value was introduced in [v0.73.0](https://github.com/VictoriaMetrics/operator/releases#release-v0.73.0), which required every rule to have both `record` and `alert` fields set. [v0.73.1](https://github.com/VictoriaMetrics/operator/releases/tag/v0.73.1) stopped identifying rules this way, since a group may contain multiple rules with the same name. The `VMRule` CRD must be updated to get this fix. See [#2622](https://github.com/VictoriaMetrics/operator/issues/2622).
